...
function LineBar({
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute("download", `${graphHeading}_LineBar.eps`);
  ...
  const downloadChart = async (format) => {
    ...
    switch (format) {
      case "png":
        ...
        link.href = image;
        link.download = `${graphHeading}_LineBar.${format}`;
        link.click();
        break;
      case "jpeg":
  ...
  labels.forEach((label, labelIndex) => {
    ...
    const color = `mycolor${labelIndex}`; // Define color for each bar
    ...
    barPlot += `
      \\addplot[
        ...
        fill=${color},
        draw=0, % border
        bar width=${barWidth}, % Set the width of the bars
      ] coordinates {
        (${label}, ${barDataPoint})
      };
    `;
    ...
    let linePlot = `
      \\addplot[
        color=green!80,
        draw=true,
        line width=1.5pt, %change line width
      ] coordinates {
    `;
    ...
    switch (format) {
      case "tex":
        ...
        const link = document.createElement("a");
        link.href = url;
        link.download = `${graphHeading}_LineBarChart.tex`;
        document.body.appendChild(link);
        link.click();
